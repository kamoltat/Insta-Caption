...
const imgModel = mongoose.model('imageGet', fileSchema);
...
async function getMusic(inputString){
    try {
        ...
        let result =  await music.trackSearch({q_lyrics: inputString, page: 1, page_size: 12, s_track_rating: "desc"});
        console.log(result);
        return result
    }
    catch(err){
...
async function getAnnotateList(results){
    const labels = await results[0].labelAnnotations;
    // const faces = await results[0].faceAnnotations;

    const result =   await Promise.all(labels.map(label => {
...
async function getFaceList(results){
    const faces = await results[0].faceAnnotations;
    const resultList = [];
    if(faces.length > 2){
         resultList.push("squad");
         resultList.push("gang");
         resultList.push("crew");
         return resultList;
    }
    else if (faces.length == 2){
        resultList.push("ride or die");
...
async function getWebList(results) {
    const webDetection = await results[0].webDetection;
    if (webDetection.webEntities.length) {
        console.log(`Web entities found: ${webDetection.webEntities.length}`);
        const result = await Promise.all(webDetection.webEntities.map(webEntity => {
            return [webEntity.score, webEntity.description];
        }));
        console.log(result);
        return [result[0][1]];
...
async function callVisionAPI(path){
    ...
    const labelResults = await client.labelDetection(path);
    const faceResults = await client.faceDetection(path);
    const webResults = await client.webDetection(path);
    const annotateList =  await getAnnotateList(labelResults);
    const faceList = await getFaceList(faceResults);
    const webList = await getWebList(webResults);
    return webList.concat(faceList,annotateList);
...
async function getUserFromMongo(id){
...
router.get('/auth/google/callback',passport.authenticate('google'),(req,res) =>{
    res.redirect('http://localhost:4200/profile/'+ req.user._id);
});

router.get('/user/:id',(req,res) =>{
    const inputString = req.params.id;
    getUserFromMongo(inputString).then((data)=>{

        res.send({id:data._id, token: data.google.token, name: data.google.name, email: data.google.email });
    })
